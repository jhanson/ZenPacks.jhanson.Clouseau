...
# package imports
from ZenPacks.jhanson.Clouseau import permissions, sessions
from urlparse import urlparse

from Products.CMFCore.utils import UniqueObject, getToolByName

from ZenPacks.jhanson.Clouseau.output import wrapper as xmlwrapper
from ZenPacks.jhanson.Clouseau.config import product_name, unique_id
from ZenPacks.jhanson.Clouseau.config import save_directory, load_directories
from ZenPacks.jhanson.Clouseau.config import enabled, enabled_only_in_debug_mode
...
if run:
    class ClouseauTool(UniqueObject,  SimpleItem):
        ...
        # login as the user
        def login(self, app, username='admin'):
        ...
        def new_session(self, context=None):
            ...
            id = sessions.get_or_create_session(namespace)
            return id
        ...
        def new_session_xml(self, context):
            ...
            return self.get_session_xml(id, context)
        ...
        security.declareProtected(permissions.Debug, "get_session_xml")
        def get_session_xml(self, session_id, context=None):
            """ Returns data on a session """
            session = self.get_session(session_id)
            ...
            self._set_content_type()
            if context and session.isReused:
                line = 'context = dmd.unrestrictedTraverse("%s")' % context
                code = session.interpreter.prepareCode(line)
                session.interpreter.runcode(code)
            return str(xml)
