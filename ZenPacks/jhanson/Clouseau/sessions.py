...
log = getLogger("ZenPacks.jhanson.Clouseau.sessions")
...
def get_or_create_session(namespace):
    """
    This is to just reuse the existing connections
    instead of creating a new one each time
    """
    session_id = 1
    if not sessions.get(session_id, None):
        sessions[session_id] = Session(namespace)
    else:
        sessions[session_id].isReused = True
    return session_id

def new_session(namespace, reuse_existing=True):
